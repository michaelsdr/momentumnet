...
from momentumnet.models import BasicBlock, MBasicBlock,\
    Bottleneck, MBottleneck, MResNet
...
def transform(model, pretrained=False, gamma=0.9, mem=False):
    """Return the MomentumNet counterpart of the model
        ...
        """
    resnet = model
    layers = [len(resnet.layer1), len(resnet.layer2),
              len(resnet.layer3), len(resnet.layer4)]
    num_classes = resnet.fc.out_features
    try:
        _ = resnet.layer1[0].conv3
        mresnet = MResNet(Bottleneck, MBottleneck,
                          layers, num_classes, gamma=gamma, mem=mem)
    except:
        mresnet = MResNet(BasicBlock, MBasicBlock,
                          layers, num_classes, gamma=gamma, mem=mem)
