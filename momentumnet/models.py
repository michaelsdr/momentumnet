...
class MResNet(nn.Module):
    ...
    def __init__(self, block, mblock, layers,
    ...
    def _make_layer(self, block, mblock, planes,
                    blocks, stride=1, dilate=False):
        norm_layer = self._norm_layer
        downsample = None
        previous_dilation = self.dilation
        if dilate:
...
def ResNet18(num_classes=1000, bn=True):
    ...
    return ResNet(BasicBlock, [2, 2, 2, 2],
                  num_classes, norm_layer=norm_layer)
...
def mResNet18(num_classes=1000, init_speed=0, gamma=.9, mem=False, bn=True):
    if bn:
        ...
        norm_layer = nn.Identity
    return MResNet(BasicBlock, MBasicBlock, [2, 2, 2, 2],
                   num_classes, init_speed=init_speed,
...
def mResNet34(num_classes=1000, init_speed=0, gamma=.9, mem=False, bn=True):
    if bn:
        ...
        norm_layer = nn.Identity
    return MResNet(BasicBlock, MBasicBlock, [3, 4, 6, 3],
                   num_classes, init_speed=init_speed,
                   gamma=gamma, mem=mem, norm_layer=norm_layer)
...
def mResNet101(num_classes=1000, init_speed=0, gamma=.9, mem=False):
    return MResNet(Bottleneck, MBottleneck, [3, 4, 23, 3],
...
def mResNet152(num_classes=1000, init_speed=0, gamma=.9, mem=False):
    return MResNet(Bottleneck, MBottleneck, [3, 8, 36, 3],
                   num_classes=num_classes,
                   init_speed=init_speed, gamma=gamma, mem=mem)
