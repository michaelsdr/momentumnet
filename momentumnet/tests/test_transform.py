...
#     tgt = torch.randn(2, 16, 512,  requires_grad=True)
#     net = nn.Transformer(num_encoder_layers=6, num_decoder_layers=6)
#     mom_net = transform(net, ['encoder.layers', 'decoder.layers'], gamma=0.99,
#                        use_backprop=True, keep_first_layer=False)
#     mom_no_backprop = transform(net, ['encoder.layers', 'decoder.layers'], gamma=0.99,
...
# #                             use_backprop=False, keep_first_layer=False)
# #
# # y = mom_no_backprop.decoder(x, tgt)
# # y = y.sum()
...
# # mom_no_backprop.eval()
# # x = torch.randn(2, 16, 512, requires_grad=True)
# # tgt = torch.randn(2, 16, 512, requires_grad=False)
# # mom_net_output = (mom_net.decoder(x, tgt) ** 2 + mom_net.decoder(x, tgt) ** 3).sum()
# # mom_output = (mom_no_backprop.decoder(x, tgt) ** 2 + mom_no_backprop.decoder(x, tgt) ** 3).sum()
# # #mom_net_output.backward()
# # #mom_output.backward()
# # params_mom_net = tuple(mom_net.decoder.parameters())
# # params_mom = tuple(mom_no_backprop.decoder.parameters())
# # grad_mom = torch.autograd.grad(mom_output, params_mom)
# # grad_mom_net = torch.autograd.grad(mom_net_output, params_mom_net)
# # for grad_1, grad_2 in zip(grad_mom_net, grad_mom):
# #     assert torch.allclose(grad_1, grad_2, atol=1e-3, rtol=1e-3)
